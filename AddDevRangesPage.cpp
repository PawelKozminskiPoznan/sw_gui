...
AddDevRangesPage::AddDevRangesPage(QWidget *parent,GenerateProjectFile *gen, vector<string>* usedProjVar, QWizard* w) : QWizardPage(parent)
{
    wizard = w;
    generate = gen;
    layout = nullptr;
...
int AddDevRangesPage::nextId() const
{
    return nextPage;
}

bool AddDevRangesPage::validatePage()
{
    ...
    {
        if(showVarInputPage())
            nextPage = MyWizard::Page_AddProjVar;
        else
...
void AddDevRangesPage::initializePage()
{
    ...
    registerField("limitPortAdd", limitPortsLineEdit);
    ++row;++row;
...
bool AddDevRangesPage::checkPortRanges(vector<string>::iterator itSection,string userData)
{
    string text;
    string actualSection = *itSection;
...
bool AddDevRangesPage::showVarInputPage()
{
    QString temp = field("comboTempAdd").toString();
    string templatePath = "templates/" + temp.toStdString();
...
string AddDevRangesPage::getErrorLog()
{
    string path;
...
void AddDevRangesPage::info(const string &error)
{
